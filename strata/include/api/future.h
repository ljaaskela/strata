...
public:
    ...
    /**
     * @brief Adds a continuation that fires when the future resolves.
     * @param callable A callable compatible with Callback (lambda, function pointer, etc.).
     * @param type Immediate fires synchronously; Deferred queues for update().
     */
    template<class F>
    void then(F&& callable, InvokeType type = Immediate)
    {
        if (future_) {
            Callback cb(std::forward<F>(callable));
            future_->add_continuation(cb, type);
        }
    }
